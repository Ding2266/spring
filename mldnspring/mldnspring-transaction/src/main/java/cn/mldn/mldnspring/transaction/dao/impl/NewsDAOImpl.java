...
public class NewsDAOImpl implements INewsDAO {
	...
	public boolean doCreate(News vo) {
		String sql = "INSERT INTO news(title,pubdate,note,price,readcount) VALUE (?,?,?,?,?)" ; 
		int len = this.jdbcTemplate.update(sql,vo.getTitle(),vo.getPubdate(),vo.getNote(),vo.getPrice(),vo.getReadcount()) ; 
		return len > 0;
	}
